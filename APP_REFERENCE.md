...
| Postgres + S3 + containerized k8s deploy | P0 | ✅ **DONE 2026-09-09** — moving off the Pi/systemd single-host deploy onto the Tamarin k3s cluster: SQLite → Postgres (`DATABASE_URL`), local `server/uploads/` → S3-compatible object storage (Garage), new `Dockerfile`/`k8s/`/`build-and-deploy.yml`. See §14.26 |
...
### 14.26 — Postgres + S3 + containerized k8s deploy  `P0`  ✅ DONE 2026-09-09

**Why.** Moving off the Raspberry Pi/systemd single-host deploy
(`deploy/climbing-app.service`, still valid for non-cluster self-hosting —
see README) onto the Tamarin k3s cluster. That cluster's model requires
stateless app pods (no PVC beyond an optional disposable cache this app
doesn't use) running as 2+ replicas, with all durable state in the
cluster's shared services instead — Postgres for the database, Garage (S3)
for object storage. SQLite (§14.3) was a local file on one disk; the local
`server/uploads/` directory (§14.5) was the same problem for images. Both
had to move.

**Schema translation, `server/db.js`.** Direct table-for-table port with a
few deliberate departures from a literal translation:
- `COLLATE NOCASE` (used for case-insensitive `username`/`setter_name`
  uniqueness) has no Postgres equivalent — replaced with a functional
  unique index, `CREATE UNIQUE INDEX ... ON table (LOWER(col))`. Every
  lookup by one of these columns had to change from `WHERE col = $1` to
  `WHERE LOWER(col) = LOWER($1)` to actually use the index and match what
  the uniqueness constraint considers a collision.
- `ascents.id` is an app-generated TEXT/UUID primary key, so unlike the
  other tables (`INTEGER GENERATED BY DEFAULT AS IDENTITY`) it has no
  natural sort order — the old schema leaned on SQLite's implicit `rowid`
  for insertion-order queries (`ORDER BY ascents.rowid`), which Postgres
  doesn't expose. Added `seq BIGINT GENERATED ALWAYS AS IDENTITY` purely to
  be `ORDER BY`-ed; a minimal, direct replacement rather than a redesign.
- `climbs.set_date` **stayed `TEXT`, deliberately not `DATE`.** The app
  already treats it as an opaque `'YYYY-MM-DD'` string
  (`isValidSetDate` validates the format itself, `currentClimbsOnly()`/
  `groupIntoCycles()` compare set dates lexically) — `node-postgres`
  round-trips a `DATE` column as a JS `Date` object by default, a
  well-documented timezone footgun for exactly this kind of string
  comparison. Kept as `TEXT` to make the risk structurally impossible
  rather than relying on remembering a driver config flag.
- `is_moderator`/`is_setter`/`is_admin`/`pass` became real `boolean`
  columns instead of SQLite's `0`/`1` integers — every write site had to
  change to `true`/`false` literals, since Postgres won't implicitly cast
  an integer to boolean on insert the way SQLite did.

**Concurrency safety had to be re-derived, not just ported.** SQLite's
`DatabaseSync` was synchronous on one shared connection — a request
handler's read-modify-write had no `await` point mid-request for another
request to interleave through, which is what made `withTransaction`'s
`BEGIN`/`COMMIT`/`ROLLBACK` sufficient on its own. `pg`'s `Pool` is async
and multi-connection, so that guarantee doesn't come free. Re-established
two ways: `withTransaction(fn)` in `server/db.js` now checks out one
pooled client for the whole transaction and passes it to `fn`, so every
statement inside genuinely runs on the same connection (a call through
`pool` directly instead of the passed-in `client` would silently run
outside the transaction — a real footgun for anyone adding a new
multi-statement mutation later); and the two places that used to rely on
"check, then insert" being race-free purely because nothing else could
interleave (signup's username check, climb creation's
`wallId`+`setterName` check) now catch `isUniqueViolation(err)` (Postgres
error code `23505`) around the transaction as the actual backstop — the
pre-check stays for a fast, friendly error in the common case, but a lost
race now surfaces as the same user-facing "already taken"/"already exists"
error instead of an unhandled 500.

**Upload storage moved to S3 (Garage on the cluster), proxied not
public.** `saveDataUrlImage` still validates/sniffs/hashes the same way,
but `PutObjectCommand`s the buffer to `S3_BUCKET` instead of
`fs.writeFileSync`-ing it (`forcePathStyle: true` on the `S3Client` —
required for Garage, it isn't real AWS). The `/uploads/<hash>.<ext>` URL
contract stored in `avatar_url`/`photo_url` and returned in API responses
is unchanged — only where the bytes physically live changed. Serving is
now a real route, `GET /uploads/:filename` (replacing
`express.static(UPLOADS_DIR, ...)`, since there's no local directory left
to serve from), which validates the filename against the exact
content-hash shape (`/^[0-9a-f]{32}\.(png|jpg|webp)$/`) *before* using it
as an S3 key — a deliberate security boundary, not just format-checking:
an unvalidated route param would let a client probe arbitrary keys in the
bucket. 404s (not 500s) on a missing object, so a stale/garbage-collected
URL doesn't read as a server error. Direct-to-bucket public URLs were
considered and rejected: Garage's S3 endpoint is only reachable from
inside the cluster's LAN, so proxying through the app is what makes an
uploaded image visible to a real user's browser at all. Dropped the old
`fs.existsSync` dedupe pre-check (skip re-writing if the content-hash
filename already exists) — an S3 `PutObject` of identical bytes to the
same key is a harmless overwrite, not worth the extra round-trip to check
first.

**Containerization/deploy.** New `Dockerfile` (multi-stage: `node:24-alpine`
builds the Vite frontend with full `devDependencies`, a second
`node:24-alpine` stage ships only prod deps + `dist/` + `server/`, runs as
the non-root `node` user, listens on `PORT=8080` to match the cluster
...
image, pushes it to GHCR, and commits the new tag into `k8s/`, which Flux
picks up — this replaces the old systemd path (`deploy/climbing-app.service`
git-pull-and-restart) as the primary deploy route on the cluster, though
that path is kept and still works for anyone self-hosting outside
Kubernetes (updated to document the now-required env vars — see README).

**Local dev.** `.env`/`.env.example` now hold `DATABASE_URL`/`S3_*` instead
of nothing (SQLite needed no configuration at all; local S3 needs a real
...
(falls back to `postgresql://postgres:postgres@localhost:5433/rockon_test`
if unset) and `ensureSchema()` in a `beforeAll`. `.github/workflows/ci.yml`
runs a `postgres:18` service container on the same port/db-name convention
so CI and local dev match; local dev's own quickest path is
`docker run --rm -d --name rockon-test-pg -p 5433:5432 -e
POSTGRES_PASSWORD=postgres -e POSTGRES_DB=rockon_test postgres:18`. Upload
tests mock the S3 client (`aws-sdk-client-mock`) rather than touching a
real bucket — `mockClient(S3Client)`, reset per test, with a default
`PutObjectCommand` resolve. `seedUsers`/`seedClimbs`/`currentUsers`/
`currentClimbs` (the test file's own seeding/read-back helpers, independent
of `server/worker.js`'s query layer) converted from synchronous
`db.prepare(...)` calls to async `pool`-based queries — every call site
across the file needed an `await` added. **324 tests passing** after both
migrations landed (up from 321 pre-upload-migration — 3 new upload-route
tests: malformed-filename 400, missing-object 404, and a streaming-body
200).

**Dead code removed as part of this**: `scripts/migrate-json-to-sqlite.js`
and its fallback stand-ins `server/users.example.json`/
`server/climbs.example.json` (§14.1) — nothing reads them at runtime any
more, and there's nothing left to migrate JSON *into* now that SQLite
itself is gone. `server/users.json`/`server/climbs.json`/
`server/orphaned-ascents.json` stay gitignored (real historical data that
may still physically sit in an old working copy) but the comment above them
in `.gitignore` no longer claims a migration script consumes them.

**Not done as part of this**: migrating cubesnail's actual production data
(their live SQLite database + uploaded photos) into the cluster, or any
DNS cutover — this section covers deploying a fresh instance of the app,
not migrating the existing one. Deliberately out of scope; see the
`.claude` conversation history around 2026-09-09 for the explicit decision
not to touch cubesnail's existing live server/data as part of this work.
